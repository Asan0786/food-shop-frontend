...
const Cart = () => {
   ...
   const{foodList,quantities,increaseQuantities,decreaseQuantites,removeFromCart} = useContext(StoreContext);

   //cart item :-> 
  const cartItems =  foodList.filter((food) => quantities[food.id] > 0);

  //calculating
  // const subtotal = cartItem.reduce((acc,food) => acc + food.price * quantities[food.id],0);
  // const shipping= subtotal === 0 ? 0.0 : 10; //if subtoal is 0 then there is no charge
                      ...
                      <div key={food.id} className="row cart-item mb-3">
                        ...
                        <div className="col-md-2">
                            <div className="input-group">
                                <button className="btn btn-outline-secondary btn-sm" type="button" onClick={()=> decreaseQuantites(food.id)}>-</button>
                                ...
                                <button className="btn btn-outline-secondary btn-sm" type="button" onClick={()=> increaseQuantities(food.id)}>+</button>
                            </div>
                        </div>
        ...
        <div className="col-lg-4">
            ...
            <div className="card cart-summary">
                <div className="card-body">
                    ...
                    <button className="btn btn-primary w-100" disabled={cartItems.length === 0 } onClick={()=> navigate('/placeOrder')}>Proceed to Checkout</button>
                </div>
            </div>
